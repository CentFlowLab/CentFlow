...
export function useSavingsAllocationAction(options?: { onSuccess?: () => void }) {
  ...
  const action = useMemo(() => {
    if (
      ...
      (liabilities?.credits ?? []).some((c) => c.outstandingBalance > 0)
    ) {
      return null;
    }
  ...
  }, [margin, assets?.goals, preferences?.prioritizeDebtAmortization, liabilities?.credits]);
  ...
  const confirmAndAllocate = useCallback(
    (allocation: SavingsAllocationAction) => {
      Alert.alert(
        'Alocar ao objetivo',
        `Reservar ${formatCurrency(allocation.amount)} para «${allocation.goalName}»?`,
        [
          { text: 'Cancelar', style: 'cancel' },
          {
            text: 'Alocar',
            onPress: () => {
              void executeAllocation(allocation).catch((error) => {
                Alert.alert('Não foi possível alocar', getApiErrorMessage(error, 'a alocação'));
              });
            },
          },
        ],
