...
export function useReceiptImage() {
  ...
  const stageDraft = useCallback((next: ReceiptDraft, source: 'camera' | 'gallery' | 'document') => {
    traceOcrStep('image_selected', { component: 'useReceiptImage', source });
  ...
  const pickFromCamera = useCallback(async () => {
    ...
    try {
      ...
      if (!result.canceled && result.assets[0]) {
        ...
        try {
          stageDraft(await buildDraftFromAsset(result.assets[0]), 'camera');
        } finally {
  ...
  const pickFromGallery = useCallback(async () => {
    ...
    try {
      ...
      if (!result.canceled && result.assets[0]) {
        setIsPreprocessing(true);
        try {
          stageDraft(await buildDraftFromAsset(result.assets[0]), 'gallery');
        } finally {
          setIsPreprocessing(false);
        }
      }
  ...
  const pickFromDocument = useCallback(async () => {
    ...
    try {
      ...
      if (!result.canceled && result.assets?.[0]) {
        setIsPreprocessing(true);
        try {
          stageDraft(await buildDraftFromDocument(result.assets[0]), 'document');
        } finally {
          setIsPreprocessing(false);
        }
      }
