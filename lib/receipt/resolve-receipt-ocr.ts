...
const EMPTY_OUTCOME: ResolveOcrOutcome = {
  result: null,
  engine: 'none',
  unavailableReason: DEFAULT_OCR_FAILED_MESSAGE,
};
...
export async function resolveReceiptOcr(
  ...
  try {
    if (inlineResult) {
      const sanitized = safeSanitizeOcrResult(inlineResult);
      return {
        result: sanitized,
    ...
    if (isSupabaseEnabled()) {
      ...
      try {
        ...
        cloud = raw ? safeSanitizeOcrResult({ ...raw, source: 'api' }) : null;
      } catch (error) {
      ...
      const device = client.result ? safeSanitizeOcrResult(client.result) : null;
      ...
      traceOcrFailure(unavailableReason ?? DEFAULT_OCR_FAILED_MESSAGE, {
        screen: 'movement_create',
        component: 'resolve-receipt-ocr',
        receiptId,
      ...
      return {
        result: device,
        engine: device ? 'device' : 'none',
        unavailableReason: unavailableReason ?? DEFAULT_OCR_FAILED_MESSAGE,
      };
    }
